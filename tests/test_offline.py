...
def test_hello_world(ixia: IxeApp) -> None:
    """ Fufu test to verify setup and connectivity"""
    print(test_hello_world.__doc__)


def test_load_config(ixia: IxeApp, locations: List[str]):
    print(test_load_config.__doc__)
    ...
    # assert(len(ixia.session.ports[self.port1].streams) == 2)
    # assert(len(ixia.session.ports[self.port2].streams) == 2)
    #
